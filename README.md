...
- **AI 新闻深读** — 每个定时时段**提前 5 分钟预生成**、随日报同步发送：抓取新闻**原文**（trafilatura 提取正文）交给 LLM，输出聊天记录风格的深度解读图（事件 / 背景 / 要点 / 影响 / 锐评）；生成上限（max_tokens）可配置——**推理模型（GLM/DeepSeek-R1 等带思考的）思考过程计入预算，建议 8000 起**，太小会截断正文导致 JSON 断裂
