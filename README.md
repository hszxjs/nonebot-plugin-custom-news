...
- **全网热点聚合** — 基于 [DailyHotApi](https://github.com/imsyy/DailyHotApi) 的 20 个源（B站/微博/澎湃/头条/IT之家/36氪/虎嗅/豆瓣/贴吧/知乎热榜等），滑块开关 + 自定义任意路由；另有**网易云 / QQ音乐新歌榜**直连官方公开接口，无需登录
- **主题化日报图** — 背景图（预设 6 张 / 上传 / 必应每日壁纸）+ ColorThief 自动取色（主色调遮罩，背景-卡片-标题浑然一体，WCAG 对比度校验）+ 毛玻璃卡片，一切可视化定制
- **AI 新闻深读** — 日报后自动跟随或命令触发：抓取新闻**原文**（trafilatura 提取正文）交给 LLM，输出聊天记录风格的深度解读图（事件 / 背景 / 要点 / 影响 / 锐评）
